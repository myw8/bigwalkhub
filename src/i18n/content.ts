...
export async function getEntryWithFallback(
  ...
  if (locale !== defaultLocale) {
    // Use the collection for the fallback lookup. Content Layer's generated
    // ID type is a finite union, so a dynamic category/slug cannot satisfy
    // getEntry's overload even though the runtime lookup is valid.
    const all = await getCollection('wiki');
    const fallback = all.find(
      (entry) => entry.id.replace(/\.mdx$/, '') === `${defaultLocale}/${category}/${slug}`,
    );
